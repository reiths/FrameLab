...
namespace details {
	// inline allows it to be defined in multiple translation units but ensures only one instance exists at link time
...
export namespace framelab::logging {
	...
	void init(path log_file) {
		...
		details::file_sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(log_file.string(), 1024 * 1024 * 5, 3);
	...
	/// 
	/// @tparam C Component type satisfying the `Component` concept (empty type with static constexpr name).
	...
	public:
		...
		template<typename... Args>
		inline static constexpr void assertion(bool result, format_string_t<Args...> fmt, Args&&... args) {
			if (!result) {
				log<level::critical>(fmt, std::forward<Args>(args)...);
		...
		/*
		template<typename... Args>
		inline static constexpr void assertion(bool result, const std::source_location& loc = std::source_location::current()) noexcept {
			if (!result) {
				log<level::err>("Precondition not fullfilled: {}, {}, line {}.", loc.function_name(), loc.file_name(), loc.line());
			}
		}
		*/
	...
	private:
		...
		static auto create_logger() -> logger_ptr {
			...
			auto logger = std::make_unique<spdlog::logger>(name, std::initializer_list{ details::console_sink, details::file_sink });
		...
		inline static constexpr auto log(format_string_t<Args...> fmt, Args &&...args) noexcept -> void {
